...
Course::Course(int id, string name, int insid, int capacity)
    : course_id(id), course_name(name), instructor_id(insid), capacity(capacity) {
}
...
void Course::saveCourses(const json& courses) {
    ...
    if (file.is_open()) file << courses.dump(4);
    else cerr << "Error: Unable to open file to save courses!" << endl;
}
